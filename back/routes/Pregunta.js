...
router.post('/:id', (req,res)=>{
	...
	if (req.files.imagen) {
		...
		ruta = req.protocol+'s://'+req.get('Host') + '/public/uploads/pregunta/'+fechab+'_'+randonNumber+'.'+extension
		fs.rename(req.files.imagen.path, path.join(__dirname, fullUrl))
	}else{
		ruta = req.protocol+'s://'+req.get('Host') + '/pregunta.png'
	}
	...
	if (req.files.imagen2) {
		...
		ruta2 = req.protocol+'s://'+req.get('Host') + '/public/uploads/pregunta/'+fechab+'_'+randonNumber2+'.'+extension2
		fs.rename(req.files.imagen2.path, path.join(__dirname, fullUrl2))
	}else{
		ruta2 = req.protocol+'s://'+req.get('Host') + '/pregunta.png'
	}
