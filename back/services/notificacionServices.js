...
class notificacionService{
	...
	getByUser(idUsuario, callback){
		notificacionSchema.find({idUsuario, activo:true}, null, {sort: {_id: -1}}).populate('idUsuario').populate('idPlan').populate('idAmigoUser').populate('idItem').populate('idUsuarioAsigna').exec(callback)
	}
	create(idUsuarioAsigna, idUsuario, tipo, idTipo, callback){
		...
		notificacionSchemas.idItem            = tipo==3 || tipo==4 ?idTipo :null
