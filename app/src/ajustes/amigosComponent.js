...
export default class ajustesAmigosComponent extends Component{
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	renderUsuarios(){
 		...
 		let newFilter = filteredData.slice(inicio, this.state.searchTerm.length==0 ?final :filteredData.length)
		...
		return filter.map((data, key)=>{
			return  <View style={style.registro} key={key} > 
						{/* <Image 
							...
							onError={ (err) => { return (<Image source={require('../assets/images/search.png')} style={style.avatarA} />) }} 
							style={style.avatarA} 
						/>  */}
						{/* <Image source={require('../assets/images/icono.png')} style={style.avatarA} /> */}
						<ImageProgress 
							resizeMode="cover" 
							renderError={ (err) => { return (<ImageProgress resizeMode="contain" source={require('../assets/images/icono.png')} imageStyle={{borderRadius:0,top:-55,left:-50, width: 100, height: 100}}  />) }} 
							source={{ uri:  data.photo}} 
							indicator={{
								size: 20, 
								borderWidth: 0,
 			...
 			if (e.estado) {
 				return(
		 			<TouchableOpacity key={key} style={style.registroAsignados} onPress={()=>navigate('profile', {userId:e._id, amigos:true})}>
						  <ImageProgress 
						  		renderError={ (err) => { return (<ImageProgress resizeMode="contain" source={require('../assets/images/icono.png')} imageStyle={{borderRadius:26,top:-26,left:-26, width: 52, height: 52}}  />) }} 
							  	source={{uri: e.photo}} 
								   style={style.avatarAsignados} 
								   imageStyle={{borderRadius:26, width: 52, height: 52}} 
								   indicator={{
									size: 20, 
									borderWidth: 0,
									color: '#ffffff',
				...
				<ScrollView>
					...
					<View style={style.contenedor}>
						<View style={style.subContenedorA}>
							...
							{
								<ScrollView style={style.contenedorLista} horizontal={true}  showsHorizontalScrollIndicator={true} onScroll={(e)=>this.onScroll(e)}>
									{this.renderUsuarios()}
								</ScrollView>
							}
							{
